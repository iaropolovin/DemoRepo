...
Процедура ПроверитьИндексНаСервере()
	Если ПолнотекстовыйПоиск.ПроверитьИндекс() Тогда
		РезультатПроверки = NStr("ru = 'Проверка индекса выполнена успешно'; sys = 'MainForm.Module.CheckResultTrue'", "ru");
		РезультатПроверкиИндекса = Новый ФорматированнаяСтрока(РезультатПроверки, , ЦветаСтиля.ЦветАкцента);
	Иначе
		РезультатПроверки = NStr("ru = 'Обнаружены ошибки при проверке индекса'; sys = 'MainForm.Module.CheckResultFalse'", "ru");
		РекомендацияПерестроения = NStr("ru = 'Рекомендуется очистить и обновить индекс'; sys = 'MainForm.Module.NeedClearAndUpdate'", "ru");
